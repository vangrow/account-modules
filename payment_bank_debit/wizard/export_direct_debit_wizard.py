...
class ExportDirectDebitWizard(models.TransientModel):
    ...
    def onchange_payment_acquirer_id(self):
        ...
        #self.payment_mode
        #raise ValidationError("Test: %s"%(self.payment_acquirer_id.provider))
        res = {
            'domain': {
                'payment_mode': domain,
            }
    ...
    def nbsf_file_generator(self):
        ...
        for invoice_id in self._context['active_ids']:
            invoice = self.env['account.move'].search(
                [('id', '=', invoice_id)])
            if invoice.state == 'posted' and invoice.partner_id.customer_payment_mode_id == self.payment_mode:
        ...
        values = {
            ...
            'description': self.payment_mode.name + ' ' + filename + ' ' + self.file_date,
        }
