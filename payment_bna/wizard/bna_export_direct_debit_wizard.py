...
class ExportDirectDebitWizard(models.TransientModel):
    ...
    def bna_file_generator(self):
        ...
        for invoice_id in self._context['active_ids']:
            ...
            if invoice.state == 'posted' and invoice.partner_id.customer_payment_mode_id == self.payment_mode_id:
                if not invoice.partner_id.bank_ids:
                    raise UserError("Error: El cliente %s de la factura %s no tiene una cuanta bancaria asociada!!!" % (
        ...
        registry_1 += self.file_date.strftime("%m")[:2] + str(
            self.file_number).rjust(2, "0")[:2]
        # 8 - Fecha imputación - tipo: numérico - long.: 8 - decimales: 0
        registry_1 += (self.file_date +
        ...
        for acc_number in registry:
            ...
            for invoice in registry[acc_number]:
                ...
                total_registry = total_registry + registry[acc_number][invoice]['amount_residual']
            # Paymen Transaction
            ...
            registry_2 += registry[acc_number][invoice]['branch_number'].rjust(4, "0")[
                :4]
            ...
            registry_2 += registry[acc_number][invoice]['company_name'].upper().ljust(10, " ")[
                :10]
