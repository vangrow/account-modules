...
class AccountMoveLineSearch(models.TransientModel):
    ...
    domain = fields.Char(
        string="Domain",
        default=lambda self: self.env['ir.config_parameter'].sudo().get_param('bank_book_domain.domain')
    )

    account_move_line_found_ids = fields.One2many(
        comodel_name='account.move.line.found',
        inverse_name='account_move_line_id',
        default=lambda self: self.env.context.get('account_move_line_found_ids', False), store=False, readonly=True,
    )
    ...
    def _check_date_to(self):
        for rec in self:
            if rec.date_to < rec.date_from:
                raise ValidationError("The Date to cannot be set in the past of Date from")
